...
- CUTLASS templates are usable as **library calls** when license / attribution
  is handled. Candidate kernel bodies still cannot copy from a tuned
  third-party kernel that builds on top of CUTLASS without authorization; the
  CUTLASS templates themselves are the canonical reference.
